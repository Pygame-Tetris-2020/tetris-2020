...
class Game:
    def __init__(self, surface):
        self.screen = surface
        self.glass = Box(self.screen, sett.BLACK, sett.glass_x, sett.glass_y, 10, 20)
        self.next_box = Box(self.screen, sett.BLACK, sett.next_box_x, sett.next_box_y, 6, 6)
        self.pause_butt = Buttons.Button()
        self.dead_cubes = []  # Создание списка неподвижных кубиков

        self.curr_fig = Figure(self.screen, 5, 0, choice(sett.colors), choice(list(sett.figure_dict)))
        self.next_fig = Figure(self.screen, 14, 4, choice(sett.colors), choice(list(sett.figure_dict)))
        ...
        self.vert_moving_delay = 500
        self.hor_control_tick = 0
        self.hor_moving_delay = 100
    ...
    def draw(self):
        ...
        self.glass.draw()
        self.next_box.draw()
        self.pause_butt.create_button(self.screen, sett.WHITE, sett.next_box_x + 3 * sett.cube_edge - 50, 420, 100, 80, 3,
                                      "Пауза", sett.BLACK)
        self.curr_fig.draw()
        ...
        text = my_font.render("Следующая:", 1, sett.BLACK)
        self.screen.blit(text, (585, 52))

        string = "Линии: " + str(self.curr_lines)
        text = my_font.render(string, 1, sett.BLACK)
        self.screen.blit(text, (585, 320))

        string = "Очки: " + str(self.curr_lines)
        text = my_font.render(string, 1, sett.BLACK)
        self.screen.blit(text, (585, 365))
    ...
    def check_events(self):
        ...
        if pygame.key.get_pressed()[pygame.K_LEFT]:
            if pygame.time.get_ticks() - self.hor_control_tick >= self.hor_moving_delay:
    ...
    def auto_moving(self):
        ...
        if can_be_moved:
            if pygame.time.get_ticks() - self.vert_control_tick >= self.vert_moving_delay:
    ...
    def destroy_lines(self):
        ...
        for i in range(1, self.glass.height + 1):
            self.dead_cubes, has_been_destroyed = self.glass.destroy_line(i, self.dead_cubes)
